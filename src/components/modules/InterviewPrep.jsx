...
function getDifficultyConfig(difficulty) {
  ...
  if (d === "standard") return { variant: "purple", color: "text-violet-600", bg: "bg-violet-50 border-violet-200" };
  if (d === "probing") return { variant: "amber", color: "text-amber-600", bg: "bg-amber-50 border-amber-200" };
  return { variant: "red", color: "text-rose-600", bg: "bg-rose-50 border-rose-200" };
}
        ...
        <div className="border-t border-cream-border bg-white/60 p-4 space-y-4">
          ...
          {question.starFramework && (
            <div>
              ...
              <div className="space-y-2.5">
                {Object.entries(question.starFramework).map(([key, value]) => (
          ...
          {question.anchorExperience && (
            <div className="bg-gradient-brand rounded-xl p-3 shadow-glow">
              ...
              <p className="text-[12px] text-white font-medium">{question.anchorExperience}</p>
            </div>
          )}
        </div>
      )}
    ...
    return (
      <div className="bg-white rounded-2xl border border-cream-border p-6 shadow-card">
        ...
        <div className="space-y-3">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} className="h-14 shimmer-bg rounded-2xl" />
          ))}
        </div>
      </div>
    );
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[12px] text-ink-muted mt-0.5">
            Predicted questions with STAR answer frameworks
          </p>
        </div>
